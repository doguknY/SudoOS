...
// #define BME280_ADDRESS 0xEC  // SDIO is grounded, the 7 bit address is 0x76
// and 8 bit address = 0x76<<1 = 0xEC
#define BME280_ADDRESS \
    0xEE  // SDIO is power, the 7 bit address is 0x77 and 8 bit address =
          // 0x77<<1 = 0xEE
 ...
 *       MODE_FORCED device goes back to sleep after one measurement. You need
 to use the BME280_WakeUP() function before every measurement
 *       MODE_NORMAL device performs measurement in the normal mode. Check
 datasheet page no 16
 *
 * @t_sb is the standby time. The time sensor waits before performing another
 measurement
 *       It is used along with the normal mode. Check datasheet page no 16 and
 page no 30
 *
...
int BMEReadRaw(void) {
    ...
    if (chipID == 0x60) {
         ...
         * Here the Pressure and Temperature are in 20 bit format and humidity
         * in 16 bit format
         */
...
void BME280_Measure(Altitude *alt) {
    if (BMEReadRaw() == 0) {
        if (tRaw == 0x800000)
            ...
            Temperature = (BME280_compensate_T_int32(tRaw)) /
...
#elif SUPPORT_32BIT
            Pressure = (BME280_compensate_P_int32(
                pRaw));  // as per datasheet, the pressure is Pa
            alt->pressure = Pressure;
        ...
        if (hRaw == 0x8000)
            ...
            Humidity = (bme280_compensate_H_int32(hRaw)) /
                       1024.0;  // as per datasheet, the temp is x1024
        }
    }
