...
void create_packet2() {
    ...
    sprintf(telemetryStrPacket,
            "419253,%d,%d,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%f,%f,%.2f,%f,%f,%.2f,%"
            "d,%.2f,%.2f,%.2f,%d,%d\n",
            packetNo, (int)gps.utc_time + 30000, altitude.pressure,
            jei.pressure, altitude.altitude, jei.altitude, altitudeDifference,
            velocity.verticalVelocity, gps.latitude, gps.longtitude,
            gps.altitude, jei.latitude, jei.longtitude, jei.altitude,
            flightState, angle.pitch, angle.roll, angle.yaw, (int)spin,
            (int)lenna.tranmissionPercentage);
}
...
void readAltitude()  // and velocity
{
    ...
    pRaw = (rawBarometerData[0] << 12) | (rawBarometerData[1] << 4) |
           (rawBarometerData[2] >> 4);
    tRaw = (rawBarometerData[3] << 12) | (rawBarometerData[4] << 4) |
           (rawBarometerData[5] >> 4);
    ...
    */
    ...
    if (velocity.timeDiffVertical >
        1.f) {  // Dikey hizi sagli olcmke icin bekleme
...
void initIMU() {
    ...
    HAL_I2C_Mem_Read_IT(&hi2c2, BNO055_I2C_ADDR_LO << 1, BNO055_ACC_DATA_X_LSB,
                        I2C_MEMADD_SIZE_8BIT, rawImuData, 24);
}
...
void readIMU() {
    ...
    rawImuData16[0] =
        ((int16_t)rawImuData[13] << 8) | ((int16_t)rawImuData[12]);
    rawImuData16[1] =
        ((int16_t)rawImuData[15] << 8) | ((int16_t)rawImuData[14]);
    rawImuData16[2] =
        ((int16_t)rawImuData[17] << 8) | ((int16_t)rawImuData[16]);
    ...
    rawImuData16[0] =
        ((int16_t)rawImuData[19] << 8) | ((int16_t)rawImuData[18]);
    rawImuData16[1] =
        ((int16_t)rawImuData[21] << 8) | ((int16_t)rawImuData[20]);
    rawImuData16[2] =
        ((int16_t)rawImuData[23] << 8) | ((int16_t)rawImuData[22]);
    ...
    if (spinFlag == 1 && angle.yaw > 0 && angle.yaw < 10 &&
        (HAL_GetTick() - spinTimeout) > 50) {
        spin++;
        spinFlag = 0;
    ...
    if (spinFlag == 2 && angle.yaw > 350 && angle.yaw < 360 &&
        (HAL_GetTick() - spinTimeout) > 50) {
        spin--;
        spinFlag = 0;
...
void initLoRa() {
    ...
    myLoRa.frequency = 420.000;          // default = 433 MHz
    ...
    if (LoRa_status != LORA_OK) buzzer(1);
...
void initSD() {
    ...
    fres = f_open(&fil, "sudo1_9.txt", FA_OPEN_APPEND | FA_WRITE | FA_READ);
...
void writeSD(char *data) {
    f_open(&fil, "sudo1_9.txt", FA_OPEN_APPEND | FA_WRITE | FA_READ);
...
int createFullPacket() {
    ...
    converter32.u32 = HAL_GetTick();
    transmitPacket[0] = converter32.u8[0];
    transmitPacket[1] = converter32.u8[1];
    transmitPacket[2] = converter32.u8[2];
    transmitPacket[3] = converter32.u8[3];

    converter32.u32 = (float)angle.roll;
    transmitPacket[4] = converter32.u8[0];
    transmitPacket[5] = converter32.u8[1];
    transmitPacket[6] = converter32.u8[2];
    transmitPacket[7] = converter32.u8[3];

    converter16.u16 = (uint16_t)angle.pitch;
    transmitPacket[8] = converter16.u8[0];
    transmitPacket[9] = converter16.u8[1];

    converter32.u32 = (float)angle.yaw;
    transmitPacket[10] = converter32.u8[0];
    transmitPacket[11] = converter32.u8[1];
    transmitPacket[12] = converter32.u8[2];
    transmitPacket[13] = converter32.u8[3];

    converter16.u16 = (uint16_t)accel.x;
    transmitPacket[14] = converter16.u8[0];
    transmitPacket[15] = converter16.u8[1];

    converter32.u32 = (float)angle.pitch;
    transmitPacket[16] = converter32.u8[0];
    transmitPacket[17] = converter32.u8[1];
    transmitPacket[18] = converter32.u8[2];
    transmitPacket[19] = converter32.u8[3];

    converter16.u16 = (uint16_t)accel.z;
    transmitPacket[20] = converter16.u8[0];
    transmitPacket[21] = converter16.u8[1];

    converter16.u16 = CRCCalculator(transmitPacket, 21);
    transmitPacket[22] = converter16.u8[0];
    transmitPacket[23] = converter16.u8[1];
